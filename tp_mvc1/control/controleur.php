...
class controleur {
	...
	public function inscription() {
		if(isset($_POST["ok"])) 
		{
			...
			{
				$lesCategories = (new categorie)->getAll();
				(new vue)->inscription($lesCategories,true,"E-mail déjà utilisé.");
			}
			elseif($_POST["motdepasse"]!=$_POST["motdepasse2"])
			{
				$lesCategories = (new categorie)->getAll();
				(new vue)->inscription($lesCategories,true,"Les mots de passes ne correspondent pas.");
			}

			else
			{
				$lesCategories = (new categorie)->getAll();
				(new vue)->inscription($lesCategories,true,"Une erreur s'est produite.");
			}
		}
		else 
		{
	...
	public function panier() {
		$lesCategories = (new categorie)->getAll();
		$lesArticles = array();
		if(isset($_SESSION["panier"])) // Toutes les infos des produits du panier seront dans cette variable
		{
	...
	public function commander() {
		if(isset($_POST["supprimer"])) {
				...
				{
					$curseur=0;
					foreach($_SESSION["panier"] as $lArticle)
					{
						if($lArticle == $_POST["supprimer"])
						{
							unset($_SESSION["panier"][$curseur]);
